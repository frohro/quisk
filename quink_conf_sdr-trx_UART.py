...
class Hardware(BaseHardware):
  def open(self):
    ...
    radio_serial_port = "/dev/ttyACM0"  # First port to try.
    radio_serial_rate = 57600  # Not nneeded for ACM serial port.  Could try commenting it out.
    # Called once to open the Hardware
    # Open the serial port.
    try:
    	self.or_serial = serial.Serial(radio_serial_port,radio_serial_rate,timeout=3)
    except serial.serialutil.SerialException:
       radio_serial_port = "/dev/ttyDummy" # Set to the port for interceptty.
    ...
    try:
        ...
        raise  # I think this stops quisk from working if there is no serial port opeened.
    ...
    version = str(self.get_parameter("VER"))  # The way the firmware is now, this sets to use the UART.
    print(version)
